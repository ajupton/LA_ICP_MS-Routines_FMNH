...
# Determine path to file, here are some examples with the samples readings 
# contained in a file named "all.xlsx"
# Absolute path if you use a Windows machine
path <- "C:/Users/uptonaw/Dropbox/LA-ICP-MS Data Backup/___Calc_ALL RESULTS/all.xlsx"

# Absolute path if you use a Mac
path <- "~/Dropbox/LA-ICP-MS Data Backup/___Calc_ALL RESULTS/all.xlsx"

# Path if your file is in your current R working directory (just the file name)
path <- "all.xlsx"

# Map to iterate read_excel over each worksheet in the workbook
ld <- path %>%
        excel_sheets() %>%
        set_names() %>%
...
# appropriate number of times for the number of samples run each day
...
# now we'll create a dataframe of those dates and add it to our sample data
...
# use stringr to get rid of repetitive element row names - which have an "X" in 
# them by default since they don't have a column name
dfnames <- df$Sample
